        ...
        <div class="container">
            <div class="row">
                <div class="col-xs-12 content-wrap">
                    ...
                    <button style="left: 5px;" class="detailbtn btn btn-dark" id="modbtn" onclick="location.href='<c:url value="/evBoard/evModi?bno=${article.bno}"/>'">수정</button>
                    </c:if>
...
$(document).ready(function() {
		...
		$('#registbtn').click(function() {
			...
			*/
			...
			$.ajax({
				type: 'post',
				url: '<c:url value="/evReply/replyRegist"/>',
		...
		function getList(reset) {
			...
			$.getJSON(
				"<c:url value='/evReply/getList/' />" + bno,
				function(data) {
					console.log(data);
		...
		$('#replyList').on('click', 'a', function(e) {
			...
			$(this).parent().children('#comModi').on('click',$(this),function(){
				...
				/* if(replyList.length = 0){
					...
					$.ajax({
						type : "post",
						url : "<c:url value='/evReply/update'/>",
						contentType : 'application/json',
						data : JSON.stringify({
							'rno' : rno,
							'content' :content
			...
			$('#replyList').on('click', 'a', function(e){
				...
				//댓글 작성자와 세션 아이디가 같다면.
					...
					if(confirm('삭제하시겠습니까?')){
						...
						$.ajax({
							type: 'post',
							url: "<c:url value='/evReply/delete' />",
							data: JSON.stringify({
								'rno' : rno,
								'writer':writer
							}),
